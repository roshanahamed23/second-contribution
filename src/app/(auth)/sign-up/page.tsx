...
const Page = () => {
  ...
  const debounced1 = useDebounceCallback(setUsername, 1000);
  const debounced2 = useDebounceCallback(setEmail, 1000);
  ...
  const onSubmit = async (data: z.infer<typeof signUpSchema>) => {
    ...
    try {
      const response = await axios.post<apiResponses>("/api/sign-up", data);
      toast({
        title: "Success",
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <p className={`text-sm ${emailMessage === "Email available!" ? 'text-green-500' : 'text-red-500'}`}>
                    {emailMessage}
                  </p>
                  <FormMessage />
                </FormItem>
